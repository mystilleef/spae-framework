...
- Read STATE.json at start; confirm `phase: "build"` and an active task
  in the cursor.
- **Mark in progress** (before Red step): write
  `cursor.task_status: "in_progress"` and
  `tasks[task_id]: "in_progress"`.
- **On task done** (after Green + Refactor pass and all verification
  steps pass):
...
- **On blocker** (unexpected red-phase result or green doesn't pass):
  write `cursor.task_status: "blocked"`, `tasks[task_id]: "blocked"`,
  append a description string to `blockers[]`. Don't advance the cursor
  or change `phase`.
- Run all task verification steps before any STATE.json write.
